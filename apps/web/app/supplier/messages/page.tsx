        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mb-6">
          ...
          <div className="flex items-center justify-between">
            <h1 className="text-2xl sm:text-3xl font-bold text-[#2b241d] flex items-center gap-3">
              <MessageSquare className="text-[#c99b4a]" />
              הודעות
              {unreadCount > 0 && (
            ...
            {unreadCount > 0 && (
              <button onClick={handleMarkAllRead} className="px-3 py-1.5 bg-[#c99b4a]/10 border border-[#c99b4a]/20 text-[#c99b4a] rounded-lg text-sm hover:bg-[#c99b4a]/20">
                סמן הכל כנקרא
              </button>
            )}
          </div>
              ...
              >
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="mt-1 flex-shrink-0">
                      {!msg.isRead ? (
                        <div className="w-3 h-3 bg-[#c99b4a] rounded-full animate-pulse" />
                      ) : (
                        <div className="w-3 h-3 bg-[#a89b8a]/30 rounded-full" />
                      )}
                    </div>

                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2 mb-1">
                        <h3 className={`font-medium text-sm truncate ${!msg.isRead ? 'text-[#2b241d]' : 'text-[#8b7c69]'}`}>
                          {msg.title}
                        </h3>
                        <span className="text-[#a89b8a] text-xs whitespace-nowrap flex items-center gap-1">
                          <Clock size={10} />
                          {new Date(msg.createdAt).toLocaleDateString('he-IL')}
                        </span>
                      </div>
                      <p className="text-[#a89b8a] text-sm line-clamp-2 whitespace-pre-wrap">{msg.message}</p>
                    </div>
                    ...
                    <div className="flex items-center gap-1 flex-shrink-0">
                      {!msg.isRead && (
                        <button
                          ...
                          className="p-1.5 rounded-lg hover:bg-[#c99b4a]/10 text-[#c99b4a] transition-colors"
                          title="סמן כנקרא"
                        >
                          <CheckCircle size={16} />
                        </button>
                ...
                {selectedMessage?.id === msg.id && (
                  <motion.div
                    ...
                    className="bg-[#f7f3f2] border border-[rgba(201,155,74,0.08)] border-t-0 rounded-b-xl p-4 -mt-1"
                  >
                    <div className="space-y-3">
                      <div>
                        <p className="text-[#a89b8a] text-xs mb-1">הודעה מלאה:</p>
                        <p className="text-[#2b241d] whitespace-pre-wrap text-sm">{msg.message}</p>
                      </div>
                      <div className="flex items-center gap-4 text-xs text-[#a89b8a]">
                      ...
                      {msg.type === 'MEETING_REQUEST' && msg.message.includes('טלפון:') && (
                        <a
                          ...
                          className="inline-flex items-center gap-2 px-4 py-2 bg-[#c99b4a]/10 border border-[#c99b4a]/20 text-[#c99b4a] rounded-lg text-sm hover:bg-[#c99b4a]/20 transition-colors"
                        >
                          <Phone size={14} />
                          התקשר עכשיו
                        </a>
