...
export default function SupplierDetailPage() {
  ...
  if (error || !supplier) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center">
        ...
        <Link href="/suppliers" className="text-[#c99b4a] hover:underline">
          חזרה לרשימת הספקים
        </Link>
      </div>
    );
  ...
  const handleShare = () => {
    ...
    Swal.fire({
      ...
      html: `
        <p style="margin-bottom: 20px; color: #8b7c69; font-size: 14px;">בחר כיצד לשתף את ${supplier.companyName}</p>
        <div style="display: flex; flex-direction: column; gap: 12px;">
          ...
          <button id="share-number" style="display: flex; align-items: center; justify-content: center; gap: 10px; padding: 14px 20px; background: rgba(201,155,74,0.06); color: #2b241d; border: 1px solid rgba(201,155,74,0.15); border-radius: 12px; font-size: 16px; font-weight: 600; cursor: pointer; transition: all 0.2s;">
      ...
      didOpen: () => {
        ...
        document.getElementById('share-number')?.addEventListener('click', () => {
          ...
          Swal.fire({
            ...
            html: `
              <p style="margin-bottom: 16px; color: #8b7c69; font-size: 15.4px; font-weight: 600;">הזן מספר טלפון לשליחת הכרטיס</p>
              <input id="swal-whatsapp-phone" class="swal2-input" type="tel" inputmode="numeric" pattern="[0-9]*" placeholder="הזן מספר טלפון" dir="ltr" style="background: #ffffff; border: 1px solid rgba(201,155,74,0.15); color: #2b241d; border-radius: 12px; text-align: center; font-size: 18px;">
            `,
            showCancelButton: true,
    ...
    Swal.fire({
      ...
      html: `
        ...
        <div style="display: flex; flex-direction: column; gap: 12px; width: 100%; max-width: 320px; margin: 0 auto;" dir="rtl">
          ...
          <label style="display: flex; align-items: center; justify-content: center; gap: 8px; padding: 16px; background: rgba(255,255,255,0.05); border: 2px dashed rgba(201,155,74,0.2); border-radius: 12px; color: #a89b8a; cursor: pointer; font-size: 14px;">
  ...
  return (
    <div className="min-h-screen">
      {/* Fixed Header */}
      <div className="fixed top-0 left-0 right-0 z-50 bg-[#f7f3f2]/80 backdrop-blur-md">
        <div className="flex items-center justify-between px-4 py-3">
          <Link href="/suppliers" className="w-10 h-10 flex items-center justify-center">
            <ArrowRight size={24} className="text-[#2b241d]" />
          </Link>
          <Image
            src="/logo1.png"
            alt="Stannel"
      ...
      <div className="relative -mt-8 mx-4 mb-4">
        <motion.div
          ...
          className="bg-white rounded-3xl overflow-hidden border border-[rgba(201,155,74,0.08)]"
        >
          {/* Company Info */}
          <div className="p-6 pb-4">
            <h2 className="text-2xl font-bold text-[#2b241d] mb-2 tracking-wide">
              {supplier.companyName}
            </h2>
            <p className="text-[#8b7c69] text-base leading-relaxed">
          ...
          <div className="px-6 pb-4">
            <div className="grid grid-cols-2 gap-2">
              {supplier.phone && (
                <a href={`tel:${supplier.phone}`} className="flex items-center gap-3 p-3 rounded-xl bg-[#f7f3f2] hover:bg-[#f0ebe8] transition-colors">
                  <div className="w-9 h-9 rounded-full bg-blue-500/20 flex items-center justify-center flex-shrink-0">
                    <Phone size={16} className="text-blue-400" />
                  </div>
                  <div>
                    <p className="text-[#a89b8a] text-[10px]">טלפון</p>
                    <p dir="ltr" className="text-[#2b241d] text-sm font-medium">{supplier.phone}</p>
                  </div>
                </a>
              )}
              {supplier.user?.email && (
                <a href={`mailto:${supplier.user.email}`} className="flex items-center gap-3 p-3 rounded-xl bg-[#f7f3f2] hover:bg-[#f0ebe8] transition-colors">
                  <div className="w-9 h-9 rounded-full bg-purple-500/20 flex items-center justify-center flex-shrink-0">
                    <Mail size={16} className="text-purple-400" />
                  </div>
                  <div className="min-w-0">
                    <p className="text-[#a89b8a] text-[10px]">מייל</p>
                    <p className="text-[#2b241d] text-sm font-medium truncate">{supplier.user.email}</p>
                  </div>
                </a>
              )}
              {supplier.address && (
                <a href={`https://maps.google.com/?q=${encodeURIComponent(supplier.address)}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-3 rounded-xl bg-[#f7f3f2] hover:bg-[#f0ebe8] transition-colors">
                  <div className="w-9 h-9 rounded-full bg-red-500/20 flex items-center justify-center flex-shrink-0">
                    <MapPin size={16} className="text-red-400" />
                  </div>
                  <div className="min-w-0">
                    <p className="text-[#a89b8a] text-[10px]">כתובת</p>
                    <p className="text-[#2b241d] text-sm font-medium truncate">{supplier.address}</p>
                  </div>
                </a>
              )}
              {supplier.website && (
                <a href={supplier.website} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-3 rounded-xl bg-[#f7f3f2] hover:bg-[#f0ebe8] transition-colors">
                  ...
                  <div>
                    <p className="text-[#a89b8a] text-[10px]">אתר</p>
                    <p className="text-[#2b241d] text-sm font-medium flex items-center gap-1">אתר אינטרנט <ExternalLink size={10} /></p>
                  </div>
                </a>
              )}
            </div>
      ...
      {galleryImages.length > 0 && (
        <motion.div
          ...
          className="mx-4 mb-6"
        >
          <h3 className="text-[#2b241d] font-semibold mb-3 flex items-center gap-2">
            גלריה
            <span className="text-[#a89b8a] text-xs">{galleryImages.length} תמונות</span>
          </h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {galleryImages.map((img: string, index: number) => (
              <motion.button
                ...
                className="relative aspect-[16/10] rounded-xl overflow-hidden group bg-[#f7f3f2]"
              >
                <ImageWithLoader
                  src={img}
                  alt={`תמונה ${index + 1}`}
      ...
      {supplier.products && supplier.products.length > 0 && (
        <motion.div
          ...
          className="mx-4 mb-6"
        >
          <div className="bg-white rounded-2xl p-4 border border-[rgba(201,155,74,0.08)]">
            <h3 className="text-[#2b241d] font-medium mb-3 flex items-center gap-2">
              מוצרים
              <span className="text-[#a89b8a] text-xs">{supplier.products.length} פריטים</span>
            </h3>
            <div className="flex gap-3 overflow-x-auto pb-2">
              {supplier.products.map((product: any) => (
                <div key={product.id} className="flex-shrink-0 w-28">
                  ...
                  <p className="text-[#2b241d] text-xs font-medium truncate">{product.name}</p>
                  <p className="text-[#c99b4a] text-xs">{(product.pointCost || 0).toLocaleString()} נק׳</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="fixed bottom-6 left-4 right-4 z-40">
        ...
        >
          ...
          <button
            ...
            className="flex-1 py-4 bg-[#c99b4a] hover:bg-[#9e7746] text-white font-semibold rounded-2xl transition-colors flex items-center justify-center gap-2 shadow-lg shadow-[#c99b4a]/20"
          >
            <Calendar size={20} />
            קביעת פגישה
          </button>
