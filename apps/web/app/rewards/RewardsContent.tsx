...
export default function RewardsContent() {
  ...
  const isArchitect = !authLoading && (user?.role === 'ARCHITECT' || user?.role === 'DESIGNER');
  const isLoggedInNonArchitect = !authLoading && user && !isArchitect;
  ...
  // Fetch wallet data only for architects/designers, after auth resolves
  useEffect(() => {
  ...
  // Show info popup for suppliers/admins
  useEffect(() => {
    if (isLoggedInNonArchitect) {
      Swal.fire({
        title: 'עמוד ההטבות',
        html: 'עמוד זה מיועד <b>לאדריכלים ומעצבים</b> בלבד.<br/>ניתן לצפות בהטבות, אך המימוש זמין רק לחברי המועדון.',
        icon: 'info',
        confirmButtonText: 'הבנתי, אצפה בהטבות',
        confirmButtonColor: '#c99b4a',
        background: '#f7f3f2',
        color: '#2b241d',
      });
    }
  }, [isLoggedInNonArchitect]);
                          ...
                          isLoggedInNonArchitect ? (
                            <button
                              onClick={(e) => {
                                ...
                                Swal.fire({
                                  title: 'מימוש הטבות',
                                  text: 'מימוש הטבות זמין לאדריכלים ומעצבים בלבד.',
                                  icon: 'info',
                                  confirmButtonText: 'הבנתי',
                                  confirmButtonColor: '#c99b4a',
                                  background: '#f7f3f2',
                              ...
                              className="flex-1 px-4 py-2.5 rounded-lg text-sm font-bold text-center border-2 border-[#c99b4a]/30 bg-[#c99b4a]/5 text-[#c99b4a]/60 cursor-default flex items-center justify-center gap-2"
                            >
                              <ShoppingCart size={16} />
                              מיועד לאדריכלים ומעצבים
                            </button>
                          ) : (
                            <Link
                              href="/login"
